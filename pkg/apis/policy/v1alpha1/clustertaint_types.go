...
type ClusterTaintPolicySpec struct {
	...
	// AddOnMatchSeconds is the duration in seconds for which the MatchConditions
	// must be continuously matched before adding the specified taint to the cluster.
	// This provides stability by ensuring the condition persists long enough
	// before taking action.
	// All MatchConditions should remain satisfied with the start time calculated
	// from each condition's lastTransitionTime.
	...
	// RemoveOnMismatchSeconds is the duration in seconds for which the MatchConditions
	...
	// The duration requirement is satisfied when any single condition is no longer met.
	// The evaluation uses the lastTransitionTime as the start point for duration calculation.
	// It requires time synchronization between the controller and the apiserver.
	// Defaults to 180.
	// +optional
