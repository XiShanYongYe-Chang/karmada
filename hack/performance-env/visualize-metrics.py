...
def parse_args():
    ...
    parser.add_argument('--current', required=True, help='Path to the current metrics JSON file')
...
class Plotter:
    def __init__(self, baseline_data, current_data, output_prefix):
        self.baseline = baseline_data
        self.current = current_data
        self.output_prefix = output_prefix
        self.page_count = 1
        self.current_subplots = [] # List of plot functions

    def add_plot(self, title, ylabel, plot_func):
        self.current_subplots.append((title, ylabel, plot_func))
        if len(self.current_subplots) >= SUBPLOTS_PER_PAGE:
            self.flush_page()

    def flush_page(self):
        if not self.current_subplots:
            return

        fig, axes = plt.subplots(ROWS, COLS, figsize=(16, 4 * ROWS))
        axes = axes.flatten()
        
        for i, (title, ylabel, plot_func) in enumerate(self.current_subplots):
            ax = axes[i]
            plot_func(ax)
        ...
        for i in range(len(self.current_subplots), len(axes)):
            fig.delaxes(axes[i])
        ...
        self.current_subplots = []
        self.page_count += 1

    def finish(self):
        self.flush_page()

def plot_latency_group(ax, baseline, current, base_path, title_suffix="Latency"):
    """
    Plots P50, P90, P99 for current, and P50, P90, P99 for baseline.
    """
    # Current
    p50 = extract_timeseries(current, base_path + ['p50_seconds'])
    p90 = extract_timeseries(current, base_path + ['p90_seconds'])
    p99 = extract_timeseries(current, base_path + ['p99_seconds'])
    ...
    # Plot Current with solid lines
    if t_p50: ax.plot(t_p50, v_p50, color='green', alpha=0.8, label='Curr P50')
    if t_p90: ax.plot(t_p90, v_p90, color='orange', alpha=0.8, label='Curr P90')
    if t_p99: ax.plot(t_p99, v_p99, color='red', alpha=0.8, label='Curr P99')

def plot_rate_group(ax, baseline, current, base_path, success_key='success', error_key='error'):
    """
    Plots Success Rate and Error Rate.
    """
    # Current
    succ = extract_timeseries(current, base_path + [success_key, 'throughput', 'rate_per_second'])
    err = extract_timeseries(current, base_path + [error_key, 'throughput', 'rate_per_second'])
    ...
    if t_succ: ax.plot(t_succ, v_succ, color='green', label='Curr Success')
    if t_err: ax.plot(t_err, v_err, color='red', label='Curr Error')

def plot_count_group(ax, baseline, current, base_path, success_key='success', error_key='error'):
    """
    ...
    succ = extract_timeseries(current, succ_path)
    err = extract_timeseries(current, err_path)
    ...
    if t_b_err: ax.plot(t_b_err, v_b_err, '--', color='pink', alpha=0.6, label='Base Error')
    if t_succ: ax.plot(t_succ, v_succ, color='green', label='Curr Total')
    if t_err: ax.plot(t_err, v_err, color='red', label='Curr Error')

def plot_depth_group(ax, baseline, current, base_path):
    """
    Plots Queue Depth (Max and Avg).
    """
    # Current
    c_max = extract_timeseries(current, base_path + ['depth', 'max'])
    c_avg = extract_timeseries(current, base_path + ['depth', 'avg'])
    ...
    # Plot Current with solid lines
    if t_c_max: ax.plot(t_c_max, v_c_max, color='red', alpha=0.8, label='Curr Max')
    if t_c_avg: ax.plot(t_c_avg, v_c_avg, color='blue', alpha=0.8, label='Curr Avg')

def plot_workqueue_count_group(ax, baseline, current, base_path):
    """
    Plots Workqueue Counts (Total Adds and Total Retries).
    """
    # Current
    adds = extract_timeseries(current, base_path + ['throughput', 'total_adds'])
    retries = extract_timeseries(current, base_path + ['throughput', 'total_retries'])
    ...
    if t_adds: ax.plot(t_adds, v_adds, color='green', label='Curr Adds')
    if t_retries: ax.plot(t_retries, v_retries, color='red', label='Curr Retries')

def plot_simple_metric(ax, baseline, current, path, label):
    curr = extract_timeseries(current, path)
    base = extract_timeseries(baseline, path)
    
    t_c, v_c = normalize_time(curr)
    t_b, v_b = normalize_time(base)
    
    if t_b: ax.plot(t_b, v_b, '--', color='gray', alpha=0.5, label=f'Base {label}')
    if t_c: ax.plot(t_c, v_c, label=f'Curr {label}')

def plot_rest_client_rate(ax, baseline, current, base_path):
    """
    Plots REST Client Rates (Total, 2xx, 4xx).
    """
    # Current
    c_total = extract_timeseries(current, base_path + ['total', 'rate_per_second'])
    c_2xx = extract_timeseries(current, base_path + ['by_status', '2xx', 'rate_per_second'])
    c_4xx = extract_timeseries(current, base_path + ['by_status', '4xx', 'rate_per_second'])
    ...
    if t_c_tot: ax.plot(t_c_tot, v_c_tot, color='blue', label='Curr Total')
    if t_c_2xx: ax.plot(t_c_2xx, v_c_2xx, color='green', label='Curr 2xx')
    if t_c_4xx: ax.plot(t_c_4xx, v_c_4xx, color='red', label='Curr 4xx')

def plot_rest_client_count(ax, baseline, current, base_path):
    """
    Plots REST Client Counts (Total, 2xx, 4xx).
    """
    # Current
    c_total = extract_timeseries(current, base_path + ['total', 'count'])
    c_2xx = extract_timeseries(current, base_path + ['by_status', '2xx', 'count'])
    c_4xx = extract_timeseries(current, base_path + ['by_status', '4xx', 'count'])
...
def plot_resource_usage(ax, baseline, current, base_path, resource_type, unit_label, scale_factor=1.0):
    """
    ...
    c_data = extract_timeseries(current, base_path + [resource_type, metric_key])
    ...
    if t_c: ax.plot(t_c, v_c, label=f'Curr {resource_type.upper()}')
...
def main():
    ...
    plotter = Plotter(baseline, current, output_prefix)
    
    # --- Policy Match ---
    plotter.add_plot("Policy Match Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'policy_match', 'latency']))
    plotter.add_plot("Policy Match Rate", "Ops/s", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'policy_match', 'throughput', 'rate_per_second'], "Rate"))
    plotter.add_plot("Policy Match Count", "Total", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'policy_match', 'throughput', 'total_count'], "Count"))

    # --- Policy Apply ---
    plotter.add_plot("Policy Apply Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'policy_apply', 'success', 'latency']))
    plotter.add_plot("Policy Apply Rate", "Ops/s", 
                     lambda ax: plot_rate_group(ax, baseline, current, ['metrics', 'policy_apply']))
    plotter.add_plot("Policy Apply Count", "Total", 
                     lambda ax: plot_count_group(ax, baseline, current, ['metrics', 'policy_apply']))

    # --- Binding Sync ---
    plotter.add_plot("Binding Sync Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'binding_sync', 'success', 'latency']))
    plotter.add_plot("Binding Sync Rate", "Ops/s", 
                     lambda ax: plot_rate_group(ax, baseline, current, ['metrics', 'binding_sync']))
    plotter.add_plot("Binding Sync Count", "Total", 
                     lambda ax: plot_count_group(ax, baseline, current, ['metrics', 'binding_sync']))

    # --- Work Sync ---
    plotter.add_plot("Work Sync Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'work_sync', 'success', 'latency']))
    plotter.add_plot("Work Sync Rate", "Ops/s", 
                     lambda ax: plot_rate_group(ax, baseline, current, ['metrics', 'work_sync']))
    plotter.add_plot("Work Sync Count", "Total", 
                     lambda ax: plot_count_group(ax, baseline, current, ['metrics', 'work_sync']))

    # --- Cluster Sync ---
    plotter.add_plot("Cluster Sync Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'cluster_sync', 'latency']))
    plotter.add_plot("Cluster Sync Rate", "Ops/s", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'cluster_sync', 'throughput', 'rate_per_second'], "Rate"))
    plotter.add_plot("Cluster Sync Count", "Total", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'cluster_sync', 'throughput', 'total_count'], "Count"))

    # --- Scheduler ---
    plotter.add_plot("Scheduler E2E Success Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'scheduler', 'e2e_scheduling', 'scheduled', 'latency']))
    plotter.add_plot("Scheduler E2E Error Latency", "Seconds", 
                     lambda ax: plot_latency_group(ax, baseline, current, ['metrics', 'scheduler', 'e2e_scheduling', 'error', 'latency']))
    plotter.add_plot("Scheduler Attempts Rate", "Ops/s", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'scheduler', 'scheduling_attempts', 'throughput', 'rate_per_second'], "Rate"))
    plotter.add_plot("Scheduler Attempts Count", "Total", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'scheduler', 'scheduling_attempts', 'throughput', 'total_count'], "Count"))
    plotter.add_plot("Scheduler Queue Incoming Rate", "Ops/s", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'scheduler', 'queue', 'incoming_bindings', 'rate_per_second'], "Rate"))
    plotter.add_plot("Scheduler Queue Incoming Count", "Total", 
                     lambda ax: plot_simple_metric(ax, baseline, current, ['metrics', 'scheduler', 'queue', 'incoming_bindings', 'total_count'], "Count"))

    # --- Controller Runtime (Dynamic) ---
    # Iterate over controllers found in the metrics
    controllers = current.get('metrics', {}).get('controller_runtime', {}).keys()
    for ctrl in sorted(controllers):
        ...
        plotter.add_plot(f"Controller: {ctrl} Latency", "Seconds", 
                         lambda ax, p=base_path: plot_latency_group(ax, baseline, current, p + ['latency']))
        
        plotter.add_plot(f"Controller: {ctrl} Rate", "Ops/s", 
                         lambda ax, p=base_path: plot_simple_metric(ax, baseline, current, p + ['throughput', 'total_rate_per_second'], "Rate"))
        
        plotter.add_plot(f"Controller: {ctrl} Count", "Total", 
                         lambda ax, p=base_path: plot_count_group(ax, baseline, current, p, 'throughput', 'error_count'))

    # --- Workqueue (Dynamic) ---
    queues = current.get('metrics', {}).get('workqueue', {}).keys()
    ...
    for q in sorted(queues):
        ...
        plotter.add_plot(f"Queue: {q} Depth", "Count", 
                         lambda ax, p=base_path: plot_depth_group(ax, baseline, current, p))
        
        plotter.add_plot(f"Queue: {q} Queue Latency", "Seconds", 
                         lambda ax, p=base_path: plot_latency_group(ax, baseline, current, p + ['queue_latency']))
        
        plotter.add_plot(f"Queue: {q} Work Latency", "Seconds", 
                         lambda ax, p=base_path: plot_latency_group(ax, baseline, current, p + ['work_latency']))
        
        plotter.add_plot(f"Queue: {q} Add Rate", "Ops/s", 
                         lambda ax, p=base_path: plot_simple_metric(ax, baseline, current, p + ['throughput', 'adds_per_second'], "Adds"))
        
        plotter.add_plot(f"Queue: {q} Counts", "Total", 
                         lambda ax, p=base_path: plot_workqueue_count_group(ax, baseline, current, p))

    # --- REST Client (Dynamic) ---
    hosts = current.get('metrics', {}).get('rest_client', {}).keys()
    ...
    for h in sorted(hosts):
        ...
        plotter.add_plot(f"REST: {h} Rate", "Ops/s", 
                         lambda ax, p=base_path: plot_rest_client_rate(ax, baseline, current, p))
        
        plotter.add_plot(f"REST: {h} Count", "Total", 
                         lambda ax, p=base_path: plot_rest_client_count(ax, baseline, current, p))

    # --- Component Resources ---
    # etcd
    plotter.add_plot("Etcd CPU Usage", "Cores", 
                     lambda ax: plot_resource_usage(ax, baseline, current, ['metrics', 'component', 'etcd'], 'cpu', 'Cores'))
    plotter.add_plot("Etcd Memory Usage", "MB", 
                     lambda ax: plot_resource_usage(ax, baseline, current, ['metrics', 'component', 'etcd'], 'memory', 'MB', scale_factor=1.0/(1024*1024)))

    plotter.add_plot("Karmada Apiserver CPU Usage", "Cores", 
                     lambda ax: plot_resource_usage(ax, baseline, current, ['metrics', 'component', 'karmada_apiserver'], 'cpu', 'Cores'))
    plotter.add_plot("Karmada Apiserver Memory Usage", "MB", 
                     lambda ax: plot_resource_usage(ax, baseline, current, ['metrics', 'component', 'karmada_apiserver'], 'memory', 'MB', scale_factor=1.0/(1024*1024)))
